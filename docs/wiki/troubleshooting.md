...
1. For legacy Maxwell/Pascal GPUs (GTX 9xx/10xx), set `open = false; legacy = true;`.
1. Check DRM modesetting: Ensure `KWIN_DRM_USE_MODIFIERS = "1"` or `NVD_BACKEND = "direct"` is set if running Wayland.
...
1. Update the `name` or `aliases` list in `platforms.desktops.niri.monitors` with the physical device identifier.
1. Reload Niri live without restarting:
   ```bash
