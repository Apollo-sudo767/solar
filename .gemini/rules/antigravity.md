...
1. **Verification**: Validate code, configuration syntax (`nix flake check` / `nix eval`), and functionality before committing.
1. **Commit & Push**: Commit changes with concise, descriptive commit messages and push the feature branch to `origin`.
1. **Pull Request**: Open a GitHub Pull Request against `main` for code review.
