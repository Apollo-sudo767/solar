...
| :--- | :--- | :--- | :--- |
| **`sol`** | The Sun (Central Star) | **Central Fleet NAS**<br>• Storage Server (NVMe + HDDs) | Samba SMB3, NFSv4, Avahi mDNS (`sol.local`), Agenix secrets, wipe-on-boot preservation, SMART diagnostics, weekly Btrfs scrub. |
...
| **`pluto`** | Dwarf Planet Pluto | **K3s HA Bootstrap Master**<br>• Lenovo M920q Tiny (32GB RAM) | Headless server, Limine, Ephemeral tmpfs root + Preservation, Agenix, K3s embedded etcd bootstrap master, weekly upgrade & reboot Sun 03:00, Tailscale mesh. |
| **`charon`** | Moon of Pluto (Charon) | **K3s HA Master Node 2**<br>• Repurposed Intel Node (16GB RAM) | Headless server, Limine, Ephemeral tmpfs root + Preservation, Agenix, K3s embedded etcd master, 50% battery cap, lid switch ignore, weekly upgrade & reboot Sun 03:30, Tailscale mesh. |
| **`hydra`** | Moon of Pluto (Hydra) | **K3s HA Master Node 3**<br>• Lenovo M920q Tiny (16GB RAM) | Headless server, Limine, Ephemeral tmpfs root + Preservation, Agenix, K3s embedded etcd master, weekly upgrade & reboot Sun 04:00, Tailscale mesh. |
